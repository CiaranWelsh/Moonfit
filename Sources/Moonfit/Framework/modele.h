...
// modele.h and modele.cpp  needs the TableCourse structure defined in tablecourse.h and tablecourse.cpp
...
/// Aim: You will need to program your own subclass of Modele, and you can use all inherited function from this mother class.
...
protected:
    int nbVars;
    int nbParams;
    vector<double> init;                                    /// initial values of variables (size bVars)
    vector<double> params;                                  /// parameter values (size nbParams)
    vector<string> names;                                   /// names of the variables
    vector<string> paramNames;                              /// names of the parameters
    vector<string> extNames;                                /// the 'official/global/external' name of each variable, when reading data / kinetics
...
public:
    ...
    virtual void finalize(){}                               /// It can be helpful that the experiments notifies the model when a simulation is finished.
                                                            /// For ODE models it might be not important, but for agent-based memory might need to be cleared
    ...
    virtual void updateDerivedVariables(double _t = 0);      /// Will be called when the solver stops at each point -> use it to calculate variables that depend on other ones
...
protected:
    ...
    double t;                                               /// advised to do 't=0;' in the initialize function
    vector<double> val;                                     /// variables at time t
    vector<double> deriv;                                   /// derivatives at time , updated each time the solver stops for output

public:
    /// In case the parameters are modified (like by optimization), it's a way to forbid to run simulations with this parameter set. (need to reload/restore a good one)
    void needNewParameterSet();

public:


/// Now, the interests of using the Modele mother class are coming :
...
/// 3 - functions for simulating (automatically calling the sub-class derivatives function, using the solver)
    ...
    virtual void simulate(double _sec_max, Evaluator* E = NULL);    /// Simulates from t (not changed), to t + _sec_max, by calling one_step
...
public:
    ...
    double print_every_dt;                                  /// frequency of saving/displaying data for kinetics
    void setPrintMode(bool z, double _print_every_dt);      /// to set the 'recording mode' to ON/OFF, and the frequency of recording
...
public:
    virtual void   setValue(string nameExternalVariable, double val);      /// to modify the value of a variable from the global ID of it
    virtual void   addValue(string nameExternalVariable, double val);      /// to modify the value of a variable from the global ID of it
    virtual double getValue(string nameExternalVariable);                  /// to get the value of a variable from its global ID
    virtual bool   isVarKnown(string externalNameVariable);                 /// to know if a variable can be simulated by the model (from its global ID)
    virtual vector<int> internValName(string externalNameVariable);             /// to know the index of a variables inside the tables
    virtual int     uniqueInternValName(string externalNameVariable);       /// same, just raises an error if there are multiple...
...
/// 11 - Internal functions
    ...
    Model(const Model &){cerr << "Checking for errors - you are copy a modele, why ?" << endl;}
...
Modele110L::Modele110L() : Modele(NBVAR, NBPARAM), background(Back::WT) {
    // overwrites the values of the generic constructor Model::Modele()
    dt = 0.2;
    print_every_dt = 1200;
