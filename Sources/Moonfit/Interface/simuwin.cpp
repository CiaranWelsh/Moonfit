...
void simuWin::identifiability(){
    ...
    motherIdentifiability(currentParamSet, chosenParameter);
...
void manageSims::motherIdentifiability(vector<double>& initialSet, int parameterIndex){
    ...
    for(int i = 0; i < NP; ++i){
        if((parameterIndex == i) || (parameterIndex < 0)){
                ...
                double lowCorrect = currentModel->getLowerBound(i);
                if(currentModel->getLowerBound(i) < 1e-12) lowCorrect = initialSet[i] / 1000;
                motherRecursiveIdentifibiality(initialSet, i, 20, true, 2, lowCorrect, currentModel->getUpperBound(i), 5);
...
string manageSims::makeTextReportParamSet(string _folder, int configuration, double simDt, double displayDt){
    ...
    tex << "\nThe function used to calculate the cost is:";
...
void simuWin::reset(){
    ...
    ListNorm << "None" << "/ Average each var" << "/ N points each var" << "/ avg and N points";
        ...
        if(ui->checkBoxAll->isChecked()){
            if(ui->checkBoxDisplayCurves->isChecked()){
                ...
                for(int i = 0; i < currentExperiment->getNbCond(); ++i){
                    ...
                    currentGraphe->Plot(2*i+1, Ys2, Yerrs2, Xs2, QString("") /*QString("Data ") + QString(currentExperiment->expName(i).c_str()) */, currentGraphe->baseList(i), Qt::SolidLine /*Qt::DashDotDotLine*/, QCPScatterStyle::ssCircle);
