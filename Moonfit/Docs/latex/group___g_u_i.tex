...
\begin{DoxyCompactList}\small\item\em Tool data structure to track the evolution of a variable with time, such as the cost during the optimization. In order to avoid reallocating a bigger table every time, it records by blocs (of size \textquotesingle{}Size\+Groups\textquotesingle{}), and everytime a block is finished, computes min, max, mean and stddev of this block and starts a new block. \end{DoxyCompactList}\item 
class \mbox{\hyperlink{classmanage_sims}{manage\+Sims}}
\begin{DoxyCompactList}\small\item\em main mother class, to perform simulations, optimizations, load configuration files, saving results as tables, etc. it has all the functions we might want to use without graphical interface. Note\+: To optimize, we give this class to the optimizer, optimizer does what he wants, changes the parameters and calls \mbox{\hyperlink{classmanage_sims_ae7da788cc26e8372b68de240ed5ca502}{get\+Cost()}} and this class records the sets it was asked to simulate (so a black optimizer can be used). \end{DoxyCompactList}\item 
class \mbox{\hyperlink{classsimu_win}{simu\+Win}}
\begin{DoxyCompactList}\small\item\em Graphical interface class. It uses all functions from the mother class \mbox{\hyperlink{classmanage_sims}{manage\+Sims}} to perform the complex tasks, but also handles the Qt graphical interface to display the settings, produce the curves, make the reports, and show the state of an optimization. \end{DoxyCompactList}\end{DoxyCompactItemize}
...
\begin{DoxyCompactList}\small\item\em In case you want to compile without using the optimizer. \end{DoxyCompactList}\end{DoxyCompactItemize}
...
{\bfseries mode\+Comparison}
 \}
\begin{DoxyCompactList}\small\item\em Modes of starting a manage\+Sim. By default, only one experiment is simulated, but one can give a multi-\/experiment class instead. The other modes mean that the original experiment has been switched by a new ecperiment that simulates perturbations of the original one, either by comparing parameter sets or by perturbating one parameter value. \end{DoxyCompactList}\end{DoxyCompactItemize}
...
\begin{DoxyCompactList}\small\item\em function defined independently of any class, to cut an interval in nb\+Points (for identifiability/sensitivity) \end{DoxyCompactList}\item 
