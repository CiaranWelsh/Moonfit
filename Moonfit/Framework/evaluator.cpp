...
void testFitnessFunction() {
    ...
    for (size_t i = 0; i < L; ++i) {
        cout << std::setprecision(8) << "Exp= " << exp[i] << ",   \tmes= " << mes[i] << "   \tcost= "
             << fitnessFunction(mes[i], exp[i], 0)
             << endl; // << "\t\tNote: bounded log " << boundedLog(exp[i]) << "," << boundedLog(mes[i]) << endl;
    }
}
...
bool Evaluator::getValsFromKinetics(TableCourse *_expectedValues, vector<string> listExtNamesVarsInModel,
                                    TableCourse *_StdDevs) {

    if (!_expectedValues) {
        cerr << "ERR GetValFromKinetics(), empty table course for expected values\n";
        return false;
    }
    ...
    if (_StdDevs)
        if (heads.size() != heads2.size()) {
            cerr << "ERR: Evaluator::getValsFromKinetics(), the two tables should have same sizes and headers";
            return false;
        }
    if (_StdDevs)
...
double Evaluator::getVal(int _Species, typeTime _time_sec, double _expectedValue, double _stdDev) {
    if (recording_before_simulation && (_expectedValue >= 0) &&
        ...
        int a = findIndex(_Species, _time_sec);
        if (a < 0) {
            cout << "   WRN : Species = " << _Species << ", t = " << _time_sec
...
void Evaluator::setValNow(int _Species, double value) {
    ...
    if (a < 0) {
        cerr << "ERR : SetValNow(" << _Species << ", ) for t=" << nextTimePoint << " not found in wish list\n";
        return;
    }
    simuData[a] = value;
