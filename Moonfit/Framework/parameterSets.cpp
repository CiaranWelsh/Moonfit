...
oneSet *pSets::getSetNumber(int i) {
    ...
    return toVector()[(unsigned int) i]; // returns in opposite order because the worst is in first position
}
...
oneParameterAnalysis::oneParameterAnalysis(vector<double> _initialParameterSet) : nbPoints(0) {
    initialParameterSet = _initialParameterSet;
    nbParams = initialParameterSet.size();
}


void oneParameterAnalysis::add(int _indexParameter, double _val, double _cost, vector<double> _costPerVariable,
                               vector<double> _costPerExperiment, vector<double> _paramSet) {
