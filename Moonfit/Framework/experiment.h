...
struct Experiment {
    ...
    Experiment(Model *_m, int _nbConditions,
               string _Identification = "");   // An experiment has to be intanciated with an existing model
    Model *m;
    ...
    virtual void simulate(int IdExp, Evaluator *E = NULL,
                          bool force = false);  // simulating one of the experiments. Do it only if the experiment is doable. If the estimator doesn't need values from this experiment, will not be simulated, unless you use force = true.
    ...
    virtual double costVariableInModel(int speciesGlobalID = -1) {
        return motherCostVarInModel(speciesGlobalID);
    }        // returns the cost of an experiment compared to data (you have to provide the data & cost yourself)
    virtual double costExternalVariable(string externalName) { return motherCostExternalVariable(externalName); }

    virtual double costExp(int ConditionID = -1) {
        return motherCostExp(ConditionID);
    }        // returns the cost of an experiment compared to data (you have to provide the data & cost yourself)
    ...
    double V(int IDexp, string Species, int time_sec) {
        return VTG[IDexp]->getVal(m->uniqueInternValName(Species), time_sec);
    }

    double expV(int IDexp, string Species, int time_sec) {
        return VTG[IDexp]->getExpVal(m->uniqueInternValName(Species), time_sec);
    }
...
struct expChangeOneParameter : public Experiment {
    ...
    expChangeOneParameter(Experiment *Exp, vector<double> &_parameterSet, int _parameterToChange,
                          int _IDconditionToUse = 0, int _nbCurves = expChangeOneParameter::NbVariantes);
