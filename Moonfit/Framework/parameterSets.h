...
/// @defgroup ParamSets Parameter Sets
/// @brief Classes to manipulate parameter sets (especially for sensitivity and identifiability)
/// @{

/// @brief 1 - *A set of parameters* is a table of parameter values and possibly a cost.
struct oneSet {
    vector<double> v;
    double cost;
...
/// @brief 3e : example function on how to use parameter sets.
void testeSets();
...
/// @brief 4b - a comparator between pointVariations (based on the cost), in order to sort them
bool ComparePointVars(pointVariation t1, pointVariation t2);

/// @brief 4c - to store the result of an analysis (identifiability or sensitivity), by playing around one parameter.
