...
struct typeVar {
    ...
    double Deg_min;
    double Deg_max;
    double Basal_Cst_min;
    double Basal_Cst_max;
    double Basal_prop_min;
    double Basal_prop_max;
    double Trans_min;
    double Trans_max;
    double ValueEQ_min;
    double ValueEQ_max;
...
enum D {
    NO_D, PROP_DEG, NB_DEG_POL
};
enum B {
    NO_B, CST, CST_FROM_EQ, NB_BAS_POL
};
...
public:
    ...
    void setBorderPolicy(
            vector<typeVar> policy); // Caution, will be read as policy[typeVariable], i.e. type is a vector index
