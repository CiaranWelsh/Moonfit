...
string Generate::generateCodeSource(){
    ...
    f << "Model" << MName << "::Model" << MName << "() : Model(NBVAR, NBPARAM), background(Back::WT) {\n";
    f << "\tdt = 0.2;\n";
    f << "\tprint_every_dt = 1200;\n";
    ...
    f << "void Model" << MName << "::setBaseParameters(){\n";
...
/// TODO
    ...
    f << "\tinitialiseDone();\n";
    f << "}\n\n";
    f << "void Model" << MName << "::derivatives(const vector<double> &x, vector<double> &dxdt, const double t){\n";
    f << generateEquations();
    f << "}\n";
    f << "#endif\n";
