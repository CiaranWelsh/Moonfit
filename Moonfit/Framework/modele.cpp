...
void Model::simulate(double sec_max, Evaluator* E){
    ...
    while(nxt < t_init + sec_max){       // when finished, 1e8
        ...
        nxt = t_init + sec_max;
        if(saveKinetics){nxt = min(nxt, print_every_dt * (double) ((int) (t / print_every_dt + 1e-8) + 1));} // the 1e-8 is due to compensate the errors. Indeed, (0.03 / 0.01) = 3 but (int) (0.03 / 0.01) was giving 2 ...
...
steps += RK4integrate( BS , val , (double) t , (double) nxt , (double) dt );     // this line can be directly replaced by the boost solver ...
        ...
        #endif
        if(saveKinetics){if( fabs((double) ((int) (t / print_every_dt + 1e-8)) - t / print_every_dt) < 1e-8) save_state(t);} // to account for division errors
    }
...
void modelAgentBased::simulate(double sec_max, Evaluator* E){
    ...
    while(nxt < t_init + sec_max){       // when finished, 1e8
        ...
        if(saveKinetics){if((E->takeDataAtThisTime(t)) || (fabs((double) ((int) (t / print_every_dt + 1e-8)) - t / print_every_dt) < 1e-8))   // 1e-8 to account for division errors
                save_state(t);
        }
    }
...
Model::Model(int _nbVars,  int _nbParams) : nbVars(_nbVars), nbParams(_nbParams), penalities(0), saveKinetics(false), currentOverrider(NULL), cinetique(NULL) {
    t = 0; stopCriterionReached = false; dt = 0.02; print_every_dt = 1200;	// basal values for simulation, can be changed by the subclass
    params.clear();         params.resize(nbParams, 0.0);
    val.clear();            val.resize(nbVars, 0.0);
    deriv.clear();          deriv.resize(nbVars, 0.0);
    names.clear();          names.resize(nbVars, string(""));
    init.clear();           init.resize(nbVars, 0.0);
    extNames.clear();       extNames.resize(nbVars, string(""));
...
void Model::initialiseDone(){
    if(! parametersLoaded) cerr << "ERR : Please provide a full parameter set to the Model before simulating. Details : you called initialise() without loading/setting parameters before initializing ! - you can not perform simulation ! Solutions : call 'loadParameters(file)', 'setParameters(full set)' or 'setBaseParameters()\n";
...
void Model::setValue(string nameExternalVariable, double value){
    ...
    for(unsigned int i = 0; i < idList.size(); ++i){if((idList[i] >= 0) && (idList[i] < nbVars)) val[idList[i]] = value;}
    if(idList.size() == 0) cerr << "ERR: Model::setValue(" << nameExternalVariable << "," << value << "), this species ID is not implanted !! \n";
    if(idList.size() > 1) cerr << "WRN: Model::setValue(" << nameExternalVariable << "," << value << "), this species ID is defined multiple times in the model. !! \n";}
void Model::addValue(string nameExternalVariable, double value){
    vector<int> idList = internValName(nameExternalVariable);
    for(unsigned int i = 0; i < idList.size(); ++i){if((idList[i] >= 0) && (idList[i] < nbVars)) val[idList[i]] += value;}
    if(idList.size() == 0) cerr << "Model::addValue(" << nameExternalVariable << "," << value << "), this species ID is not implanted !! \n";
    if(idList.size() > 1) cerr << "WRN: Model::setValue(" << nameExternalVariable << "," << value << "), this species ID is defined multiple times in the model. !! \n";}
double Model::getValue(string nameExternalVariable){
    vector<int> idList = internValName(nameExternalVariable);
    if(idList.size() > 1) {cerr << "WRN: Model::getValue(" << nameExternalVariable << "), this species ID is defined multiple times in the model. !! -> Can not decide which one to get \n";
        return NAN;}
    if(idList.size() == 0){cerr << "Model::getValue(" << nameExternalVariable << "), this species ID is not implanted !! \n";
        return NAN;}
    int id = idList[0]; if((id >= 0) && (id < nbVars)) return val[id]; else return NAN;}
...
int Model::uniqueInternValName(string externalNameVariable){ // just raises an error if there are multiple...
    ...
    if(res.size() == 0) {cerr << "ERR: uniqueInternValName(" << externalNameVariable << "), not found" << endl; return -1;}
    if(res.size() > 1) {cerr << "ERR: uniqueInternValName(" << externalNameVariable << "), defined multiple times" << endl; return -1;}
    return res[0];}
int Model::getNbParams(){return nbParams;}
void Model::setParam(int i, double v){
    if((i < 0) || (i >= nbParams)) {cerr << "Model::setParam(" << i << "," << v << "), index out of bounds. NbParams = " << nbParams << endl; return;}
    params[i] = (v > 0? v : -v);}   // fuck the system !!
double Model::getParam(int i){
    if((i < 0) || (i >= nbParams)) {cerr << "Model::getParam(" << i << "), index out of bounds. NbParams = " << nbParams << endl; return 0.0;}
...
void Model::setParameters(vector<double> &newParamSet){
    if((int) newParamSet.size() != nbParams) {cerr << "ERR- Model::setParameters(vector), wrong size for the given vector (" <<  newParamSet.size() << ") instead of " << nbParams << " parameters\n"; return;}
    for(int i = 0; i < nbParams; ++i){params[i] = newParamSet[i];}
    parametersLoaded = true;
}

string Model::getParamName(int i){ if((i < 0) || (i >= nbParams)) {cerr << "ERR- Model::getParamName(" << i << "), out of bounds (" << nbParams << " params)" << endl; return "";} else return paramNames[i];}
double Model::getLowerBound(int i){ if((i < 0) || (i >= nbParams)) {cerr << "ERR- Model::getLowerBound(" << i << "), out of bounds (" << nbParams << " params)" << endl; return 0;} else return paramLowBounds[i];}
double Model::getUpperBound(int i){ if((i < 0) || (i >= nbParams)) {cerr << "ERR- Model::getUpperBound(" << i << "), out of bounds (" << nbParams << " params)" << endl; return 0;} else return paramUpBounds[i];}
void Model::setBounds(int i, double vLow, double vHi){ if((i < 0) || (i >= nbParams)) {cerr << "ERR- Model::setBound(" << i << "), out of bounds (" << nbParams << " params)" << endl; return;} else {paramLowBounds[i] = vLow; paramUpBounds[i] = vHi;}}
void Model::setBounds(vector<double> lowVals, vector<double> upVals){
    if(((int) upVals.size() != nbParams) || ((int) lowVals.size() != nbParams)) {cerr << "ERR- Model::setBounds(vector1, vector2), the sizes of vectors don't match nbParams = " << nbParams << endl; return;}
    paramLowBounds = lowVals;
    paramUpBounds = upVals;}

void Model::action(string name, double parameter){cerr << "ERR- Model::action(string, double), this function should and was not implemented in the Model subclass\n";}
void Model::action(string name, vector<double> parameters){cerr << "ERR- Model::action(string, vector<double>), this function should and was not implemented in the Model subclass\n";}
...
void Model::clearOverride(vector<double> &x, vector<double> &dxdt){
    if(currentOverrider){
        ...
        for(int i = 0; i < nbVars; ++i){
            string z = extNames[i];
            if(z.size() > 0) if((*currentOverrider)(z)) {x[i] = 0.0; dxdt[i] = 0.0;}
        }
    }
}
