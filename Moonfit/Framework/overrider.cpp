...
overrider::overrider(TableCourse *kinetics, bool useSplines): nbIndices(0) {
    nbIDsGiven = 0;
    if(!kinetics) {cerr << "ERR: Overrider created from an empty pointer to TableCourse" << endl; return;}
...
void overrider::learnSpl(key nameVar, vector<double> xs, vector<double> ys, bool yesSplnoLinearInterpol){
    if(hasData(nameVar)) {cerr << "WRN : overrider::learnSpl, data was already saved for  " << nameVar << "; data overrided.\n";}
    int index = getID(nameVar);
...
void overrider::setOver(key nameVar, bool value){ // put string("") to mean all
    ...
    if((!hasData(nameVar)) && (value))   {cerr << "ERR : overrider::setOver(" << nameVar << ", value = " << (value ? "true" : "false") << "), this index does not refer to any stored data (no data for this index)\n"; return;}
    override[index] = value;
}


void overrider::learnFunct(key nameVar, double (*f) (double)){
    if(hasData(nameVar)) {cerr << "WRN : overrider::learnFunct, data was already saved for " << nameVar << "; data overrided.\n";}
    int index = getID(nameVar);
