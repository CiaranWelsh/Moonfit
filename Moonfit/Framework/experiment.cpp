...
Experiment::Experiment(Model *_m, int _nbConditions, string _Identification) : m(_m), nbConditions(_nbConditions),
...
double Experiment::motherCostExternalVariable(string extName) {
    ...
    double res = 0;
    vector<int> list = m->internValName(
            extName); // multiple variables might simulate the same data... just allows it even if it's weird...
    if (list.size() > 1) {
...
string Experiment::print() {
    ...
    for (int i = 0; i < nbConditions; ++i) {
        res << "\t" << i << "\t"
            << getConditionName(i); // << (isDoable(i) ? "\tCan Be Simulated\n" : "\tNot doable with this model\n");
    }
    for (int i = 0; i < nbConditions; ++i) {
...
string MultiExperiments::BigExpName(int i) {
    if ((i >= 0) && (i < nbBigExp()))
...
double MultiExperiments::costBigExp(int BigExpID) {
    ...
    if (BigExpID < 0) {
        for (int i = 0; i < NbBigExps; ++i) {
            res += coefficients[i] *
                   ListBigExperiments[i]->costVariableInModel();        // careful, mixing between indices and enum in
        }
    } else {
...
expCompParameterSets::expCompParameterSets(Experiment *Exp, vector<vector<double> *> _parameterSets,
...
expChangeOneParameter::expChangeOneParameter(Experiment *Exp, vector<double> &_parameterSet, int _parameterToChange,
                                             int _IDconditionToUse, int _nbCurves) :
